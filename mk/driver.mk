...
DOCKER_MACHINE_STORAGE := $(HOME)/.docker/machine
DOCKER_MACHINE_VM_NAME := qemu-test
...
driver-run: clean build install driver-remove
	$(DOCKER_MACHINE_CMD) --storage-path $(DOCKER_MACHINE_STORAGEPATH) create --driver qemu \
		--qemu-cpu-count $(DOCKER_MACHINE_VM_CPU_COUNT) \
		--qemu-memory $(DOCKER_MACHINE_VM_MEMORY_SIZE) \
		--qemu-disk-size $(DOCKER_MACHINE_VM_DISKSIZE) \
